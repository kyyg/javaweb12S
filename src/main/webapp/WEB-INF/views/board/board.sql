...
create table board (
	...
	fixed   char(3) default 'off',
	part   varchar(20) not null,     /* 문의 카테고리 분류(상품/배송/취소환불/기타...) */
	pwd    varchar()100) not null, 		/* 비밀번호(스프링 시큐리티(BCryptPasswordEncoder) 처리) */
	answer char(4) not null default '미답변'  /* 문의글 답변 여부 (미답변/답변완료) */
	primary key(idx)
);
...
/* <![CDATA[조건식]]> 부등호(<,>) 오류날수 있어서 쓰는 방식 */ <![CDATA[조건식]]>
select idx, title from board where idx in (
	(select idx from board where idx < 6 order by idx desc limit 1),
	(select idx from board where idx > 6 limit 1));
